...
class IMDbMoviesPlot():
    def __init__(
        ...
        self.vector_store = vector_store
        self.path = path
        self.data = pd.read_csv(self.path)[["Title", "Plot"]]
        self.movies_with_uuid = self.define_movies()
        self.text_splitter = self.select_text_splitter(
            chunking_strategy=chunking_strategy,
            chunk_size=chunk_size,
    ...
    def define_movies(self) -> dict[str, str]:
        ...
        """
        return {
            movie: str(uuid4()) for movie
            in list(self.data["Title"].unique())
        }
    ...
    def add_uuid_to_df(self) -> None:
        ...
        Returns:
            None.
        """
        self.data['Title_uuid'] = self.data['Title'].map(self.movies_with_uuid)
